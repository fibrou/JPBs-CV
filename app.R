...
ui <- fluidPage(
  tags$head(tags$style(HTML("
    ...
    .gen-status { font-family: monospace; background: #f5f5f5;
  ...
  tabsetPanel(
    ...
    # ── Generate CV tab ────────────────────────────────────────────────────────
    tabPanel("⚙ Generate CV",
      br(),
      h3("Generate & Download CV"),
      p("Renders the CV from the current database contents. Takes about 30–60 seconds."),
      p(tags$em("Tip: make all your edits first, then click Render once.")),
      hr(),
      fluidRow(
        column(3,
      ...
      br(),
      div(class = "gen-status", textOutput("render_status"))
    )
  )
)
...
server <- function(input, output, session) {
  ...
  # ── Data editing modules (one per table) ──────────────────────────────────
  ...
  # ── Generate CV ────────────────────────────────────────────────────────────
  pdf_path <- here("jpbDataDrivenCV.pdf")

  rv_rendered <- reactiveVal(FALSE)
  rv_status   <- reactiveVal({
    if (file.exists(pdf_path))
      paste("Last render:", format(file.mtime(pdf_path), "%d %b %Y %H:%M"))
    else
      "No PDF found yet. Click Render to generate."
  })

  output$download_ui <- renderUI({
    if (isTRUE(rv_rendered())) {
      downloadButton("download_cv", " Download PDF",
                     icon  = icon("file-pdf"),
                     class = "btn-success btn-lg",
                     style = "width:160px")
    } else {
      tags$button(" Download PDF",
                  class = "btn btn-success btn-lg",
                  style = "width:160px;opacity:0.45;cursor:not-allowed",
                  disabled = NA)
    }
  })

  observeEvent(input$render_cv, {
    rv_status("Preparing data...")
    rv_rendered(FALSE)

    withProgress(message = "Rendering CV...", value = 0, {

      # Step 1: export all 27 tables to a single RDS file ─────────────────────
      setProgress(0.1, detail = "Exporting database...")
      ...
      export_result <- tryCatch({
        ...
        tbl_map <- list(
          ...
          honorsGrants           = "honors_grants",
          publications           = "publications",
          pip                    = "papers_in_progress",
          otherPubsMonos         = "other_pubs_monos",
          academicPresents       = "academic_presentations",
          pedagogyPresents       = "pedagogy_presentations",
          academicDiscussions    = "academic_discussions",
          confCommittees         = "conf_committees",
          confChair              = "conf_chair",
          editor                 = "editor",
          adHocReview            = "ad_hoc_review",
          tAndP                  = "t_and_p",
          profesionalOFfices     = "professional_offices",
          academicLeader         = "academic_leadership",
          academicCommittees     = "academic_committees",
          dissertationCommittees = "dissertation_committees",
          thesesSupervision      = "theses_supervision",
          studentMentoring       = "student_mentoring",
          profTraining           = "prof_training",
          mediaInterviews        = "media_interviews",
          bizExperiences         = "biz_experiences",
          references             = "references"
        )

        cv_data <- lapply(tbl_map, function(tbl) dbReadTable(con, tbl))
        saveRDS(cv_data, here("cv_data.rds"))
        "ok"
      }, error = function(e) conditionMessage(e))

      if (!identical(export_result, "ok")) {
        rv_status(paste("Export failed:", export_result))
        return()
      }

      # Step 2: render in a clean subprocess ───────────────────────────────────
      setProgress(0.2, detail = "Knitting document...")
      rv_status("Rendering — please wait (30–60 s)...")

      rmd <- normalizePath(here("jpbDataDrivenCV.Rmd"))
      pdf <- normalizePath(pdf_path, mustWork = FALSE)

      result <- tryCatch({
        callr::r(
          function(rmd, pdf) rmarkdown::render(rmd, output_file = pdf, quiet = TRUE),
          args = list(rmd = rmd, pdf = pdf)
        )
        "ok"
      }, error = function(e) conditionMessage(e))
    ...
    if (identical(result, "ok")) {
      rv_rendered(TRUE)
      rv_status(paste("CV rendered", format(Sys.time(), "%d %b %Y %H:%M"),
                      "— click Download to save."))
    } else {
      rv_status(paste("Render failed:", result))
    }
  })

  output$render_status <- renderText(rv_status())

  output$download_cv <- downloadHandler(
    filename = function() {
      paste0("JPBroussard_CV_", format(Sys.Date(), "%Y%m%d"), ".pdf")
    },
